...
let source = "wks";
let kind = `${testUtils.partition}:${source}:Manifest:1.0.0`;
...
describe(scenario, (done) => {
    ...
    describe('Scenario: Manifest Ingest', (done) => {
        ...
        if (config.test_flags.scenario_manifest_ingest.enableScenario) {
          ...
          describe('Manifest Ingest #TC-5: Search for Wellbores with WelboreID=', (done) => {
            ...
            it("Search: Record", done => {
              ...
              test.expectedResponse = test.api.expectedResponse;
              let searchQuery = {
                "kind": `${testUtils.partition}:${source}:work-product-component--WellboreMarkerSet:1.0.0`,
