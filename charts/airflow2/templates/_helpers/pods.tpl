...
EXAMPLE USAGE: {{ include "osdu.airflow.volumes" (dict "Release" .Release "Values" .Values.airflow "extraPipPackages" $extraPipPackages "extraVolumes" $extraVolumes) }}
*/}}
{{- define "osdu.airflow.volumes" }}
{{- /* airflow_local_settings.py */ -}}
{{- if or (.Values.airflow.airflow.localSettings.stringOverride) (.Values.airflow.airflow.localSettings.existingSecret) }}
- name: airflow-local-settings
  secret:
    {{- if .Values.airflow.airflow.localSettings.existingSecret }}
    secretName: {{ .Values.airflow.airflow.localSettings.existingSecret }}
    {{- else }}
    secretName: {{ include "osdu.airflow.fullname" . }}-local-settings
    {{- end }}
    defaultMode: 0644
{{- end }}
...
- name: dags-data
  persistentVolumeClaim:
    ...
    {{- else }}
    claimName: {{ printf "%s-dags" (include "osdu.airflow.fullname" . | trunc 58) }}
    {{- end }}
{{- else if .Values.airflow.dags.gitSync.enabled }}
- name: dags-data
  emptyDir: {}
...
- name: logs-data
  persistentVolumeClaim:
    ...
    claimName: {{ .Values.airflow.logs.persistence.existingClaim }}
    {{- else }}
    claimName: {{ printf "%s-logs" (include "osdu.airflow.fullname" . | trunc 58) }}
    {{- end }}
{{- end }}
...
- name: git-secret
  secret:
    secretName: {{ .Values.airflow.dags.gitSync.sshSecret }}
    defaultMode: 0644
{{- end }}
{{- if .Values.airflow.dags.gitSync.sshKnownHosts }}
- name: git-known-hosts
  secret:
    secretName: {{ include "osdu.airflow.fullname" . }}-known-hosts
    defaultMode: 0644
{{- end }}
{{- end }}
...
{{ toYaml .Values.airflow.airflow.extraVolumes }}
{{- end }}
